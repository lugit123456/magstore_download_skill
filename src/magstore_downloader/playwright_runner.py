...
class MagStoreRunner:
    ...
    def _run_magazine_once(self, page, magazine: MagazineConfig, timeout_error_type) -> MagazineRunResult:
        ...
        if downloaded_path is None:
            # 文件已存在且配置为跳过
            return self._checked_skip(magazine, issue, now, "file-exists-skip")
    ...
    def _download_issue(self, page, magazine: MagazineConfig, issue: IssueInfo) -> Optional[Path]:
        magazine_dir = self.config.download.base_dir / magazine.download_subdir
        ...
        if target is None:
            self.logger.info("文件已存在且 filename_conflict=skip，跳过下载: %s", magazine_dir / safe_name, extra=extra(magazine.id, "download"))
            return None
